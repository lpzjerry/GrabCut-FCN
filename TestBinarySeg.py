...
def find_superpixels_under_marking(marking, superpixels):
    fg_segments = np.unique(superpixels[marking[:, :, 2] == 255])
    bg_segments = np.unique(superpixels[marking[:, :, 2] == 219])
...
def Graph_cut_attractive(img_marking, fname):
    ...
    cv2.imwrite('binary_seg/mask-test/' + fname, mask)
    return mask
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) != 3):
        ...
        sys.exit()
    '''
    images = [cv2.imread(file) for file in glob.glob("Data/images-test/*.png")]
    images_anno = [cv2.imread(file) for file in glob.glob("Data/images-labels-test/*.png")]
    
    fnames = [glob.glob("Data/images-test/*.png")]
    fnames = list(itertools.chain.from_iterable(fnames))
    fnames_anno = [glob.glob("Data/images-labels-test/*.png")]
    fnames_anno = list(itertools.chain.from_iterable(fnames_anno))
    print(len(images))
    
    
    for i in range (0,30):
        #img = cv2.imread('Data/png/images/21077.png', cv2.IMREAD_COLOR)
        ...
        centers, color_hists, superpixels, neighbors = superpixels_histograms_neighbors(
            ...
            #img_marking = cv2.imread('Data/images-labels/21077-anno.png', cv2.IMREAD_COLOR)
        img_marking = images_anno[i]
        Graph_cut_attractive(img_marking,fnames[i][16:])
